...
% keys
global CATE1_KEY CATE2_KEY categories
...
for tr = 1:length(trial_mat.trial)
    ...
    if strcmp(trial_mat.task{tr}, 'categorization')
        ...
        if (trial_mat.trial_response(tr) == CATE1_KEY && strcmp(trial_mat.category{tr}, categories{1})) ||...
                (trial_mat.trial_response(tr) == CATE2_KEY && strcmp(trial_mat.category{tr}, categories{2}))
            trial_mat.trial_accuracy(tr) = 1;

        elseif (trial_mat.trial_response(tr) == CATE2_KEY && strcmp(trial_mat.category{tr}, categories{1})) ||...
                (trial_mat.trial_response(tr) == CATE1_KEY && strcmp(trial_mat.category{tr}, categories{2}))
