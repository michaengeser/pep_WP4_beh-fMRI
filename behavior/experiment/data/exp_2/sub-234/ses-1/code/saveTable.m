...
global sub_num session category task exp_num


% make files names 
if length(category) > 1
    dir = string(fullfile(pwd, 'data', ['exp_', num2str(exp_num)],...
        ['sub-', num2str(sub_num)], ['ses-',num2str(session)], task, category));
    fileName = fullfile(dir, ['sub-', num2str(sub_num), '_ses-', num2str(session),...
        '_run-', num2str(blk_num), '_task-', task, '_', category, '_events']);
else
    dir = string(fullfile(pwd,'data',['exp_', num2str(exp_num)],...
        ['sub-', num2str(sub_num)], ['ses-',num2str(session)], task));
    fileName = fullfile(dir, ['sub-', num2str(sub_num), '_ses-', num2str(session),...
