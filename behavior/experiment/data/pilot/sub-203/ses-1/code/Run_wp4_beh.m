...
% To get same seeds for matlab randomization functions.
rng(1);

% global parameters:
global sub_num TRUE FALSE refRate task SHOW_PRACTICE session exp_num
global FRAME_ANTICIPATION PHOTODIODE DIOD_DURATION SHOW_INSTRUCTIONS category categories
global RESTART_KEY NO_KEY spaceBar valid_resp_keys kb
global CATE1_KEY CATE2_KEY
global expDir
...
if exp_num == 2
    task_num = 1; % in the experiment 2 only categorization will be performed
else
...
[tr_mat, file_list] = load_trial_matrix(sub_num,1,exp_num);
...
[tr_mat] = durationXrefRate(tr_mat);
...
try
 ...
 %% check if participant and session exists already
            ...
            if strcmp(task, 'categorization')
                ...
                valid_resp_keys = [CATE1_KEY, CATE2_KEY];
            end
