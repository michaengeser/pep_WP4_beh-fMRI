function d = compare_task_RDMs_to_multiple_predictor_RDMs(d, cfg)
%
...
% loop through tasks
for task_i = 1:numel(cfg.tasks_of_interest)
    voi = char(cfg.tasks_of_interest(task_i));
    cfg.currentVOI = voi;
        ...
        for row = 1:height(res_table)
            ...
            if cfg.permutation_test
                ...
                res_table.p_val_cate1(row) = ...
                    sum(d.compare_task_to_predictor.permutation_test.(voi).(cfg.categories{1})(row,:)...
                    >= res_table.r_val_cate1(row)) / cfg.n_permutations;
                res_table.p_val_cate2(row) = ...
                    sum(d.compare_task_to_predictor.permutation_test.(voi).(cfg.categories{2})(row,:)...
...
if cfg.plotting
    ...
    for task_i = 1:numel(cfg.tasks_of_interest)
        voi = char(cfg.tasks_of_interest(task_i));
        all_p_vals(:, task_i) = d.compare_task_to_predictor.(voi).category_average.p_val;
    end
    ...
    for i_pred = 1:height(res_table)
        ...
        for task_i = 1:numel(cfg.tasks_of_interest)
            voi = char(cfg.tasks_of_interest(task_i));
            d.compare_task_to_predictor.(voi).category_average.p_val(i_pred) = fdr_pval(task_i);
            disp(['FDR corrected p value for ', voi, ': ', num2str(fdr_pval(task_i)),...
                ' ', char(all_asterisks(i_pred, task_i))])
            disp(['R value for ', voi, ': ', ...
                num2str(d.compare_task_to_predictor.(voi).category_average.r_val(i_pred))])
        end
    end
